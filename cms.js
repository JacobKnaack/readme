const articles = [
  {
    ...
    "html": "<p class='subtitle'>This post is focused on basic web development and design methodology. The goal is to help people take a prototype from concept to a development build running on a local computer. But the build will be simple enough to push to any hosting service without any fancy tools.</p>\
            ...
            <div class='divider'></div>\
            <h3 class='contentHeading'>Now we can Create our files</h3>\
            <p>Now create a folder or directory anywhere on your computer, name it whatever you like, and create three files in that directory, one titled index.html, another titled style.css, and a third titled chat.js. Here's my directory structure after my files are created:</p>\
            ...
            <p>Now Lets open up our html file in a text editor of your choosing and begin laying out our template for our chat interface:</p>\
            <p></p>"  
  },
];
...
for ( var item in articles) {
  if (document.getElementById("submissions")) {
    document.getElementById("submissions").innerHTML += "<li class='indexListItem'><a id='article" + articles[item].id + "' class='listAnchor' href='"+url+"template/article.html?id=" + articles[item].id + "'><i class='fa fa-code indexListImg' class aria-hidden='true'></i>"+articles[item].name+"</a></li>"
  }
